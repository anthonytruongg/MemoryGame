...
const CardGrid = (props) => {
    ...
    const colorsArray = ['blue', 'red', 'green', 'purple', 'pink', 'indigo', 'orange', 'teal', 'yellow']
    ...
    <div className='flex flex-row my-10 justify-center'>
     ...
     <div className="grid grid-cols-3 gap-10">
        {colorsArray.map((displayColor, index) => {
            return (
                <div key={index} className={`bg-${displayColor}-500 h-20 w-20 py-7 flex justify-center rounded-3xl text-${displayColor}-100 font-bold
                ...
                `
                } onClick={handleClick}>{displayColor}</div>
